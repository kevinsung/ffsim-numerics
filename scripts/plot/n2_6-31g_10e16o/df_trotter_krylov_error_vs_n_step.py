...
for time_step in time_step_range:
    for order in order_range:
        for trotter_n_steps in trotter_n_steps_range:
            ...
            assert all(errors > 0)
            data[time_step, order, trotter_n_steps] = errors
    ...
    exact_data[time_step] = np.abs(exact_krylov_errors[: krylov_n_steps + 1])
...
for time_step in time_step_range:
    for order in order_range:
        ...
        ax.plot(range(2, krylov_n_steps + 3), exact_data[time_step], label="exact")
        for trotter_n_steps, color in zip(trotter_n_steps_range, colors):
            errors = data[time_step, order, trotter_n_steps]
            ax.plot(
                range(2, krylov_n_steps + 3), errors, label=f"n_steps={trotter_n_steps}"
            )
        ...
        filename = os.path.join(
            plots_dir,
            f"{os.path.splitext(os.path.basename(__file__))[0]}_time_step-{time_step}_order-{order}.pdf",
        )
