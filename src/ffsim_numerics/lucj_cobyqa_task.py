...
def run_lucj_cobyqa_task(
    task: LUCJCOBYQATask,
    *,
    data_dir: Path,
    molecules_catalog_dir: Path,
    bootstrap_task: LUCJCOBYQATask | None = None,
    ...
    def fun(x: np.ndarray) -> float:
        ...
        logger.debug(f"Computing state vector took {t1 - t0} seconds.")

        t0 = timeit.default_timer()
        energy = np.vdot(final_state, hamiltonian @ final_state).real
        t1 = timeit.default_timer()
        logger.debug(f"Computing energy took {t1 - t0} seconds.")
